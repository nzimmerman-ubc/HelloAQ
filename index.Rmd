...
# Package names
packages <- c("tidyverse", "plyr", "dplyr", "lubridate", "leaflet", "RColorBrewer")
...
current_conc <- current_conc %>% mutate(RAMP_IDs = RAMP_IDs)
```

The outcomes of this research will help the UBC community learn about their air pollution exposure in real-time. Click on this map below to find out more about your personal air pollution exposure at different locations on campus. The points are col

```{r Pollution_Map, echo=FALSE}
AQ_df <- join(current_conc,RAMP_info)

AQ_df$severity = cut(AQ_df$PM25_RAMP,
                             breaks=c(0,10,25,100,1000), right=FALSE,
                             labels=c("<10","10-25","25-100","100+"))

palette_rev <- rev(brewer.pal(4, "RdYlBu"))
pal <- colorFactor(
  palette = palette_rev,
...
# Reading the Map
Click on the individual sensors to read the respective pollutant concentrations across campus. Size of the circle increases as the PM~2.5~ concentration increases. Breathing high concentrations of PM~2.5~ can lead to chronic illnesses including lung cancer and increased risk of heart attacks. 
...
# References 
1.	Environment Canada, 2011, Criteria Air Contaminants and Related Pollutants, www.ec.gc.ca/Air/default.asp?lang=En&n=7C43740B-1 (accessed August 5, 2021).
2.	https://numina.co/
3.	https://bluecity.ai/
